...
class Mathcad:
    ...
    def __init__(self, f=math.tan(30*3.14/180), u=1, C = 1):
        self.f = f
        self.u = u
        self.C = C
    ...
    def calcSymbWithAngle2(self, Point1, Point2, pointFind):
        x = sym.Symbol('x')
        y = sym.Symbol('y')
        p = sym.Symbol('p')
        q = sym.Symbol('q')
        ...
        # [((a + n) / 2), ((b + m) / 2), ((c + g) / 2), ((d + h) / 2 - 0.1)],
        # answer = sym.nsolve([calc1, calc2, calc3, calc4, calc5],
        #                     [x, y, p, q, alpha],
        #                     [-((a+n)/2), -((b+m)/2), -((c+g)/2), -((d+h)/2-0.1), -((Point1.getAlpha(f)+Point2.getAlpha(f))/2-0.1)],
        #                     verify = False)
        answer = sym.solvers.nsolve([calc1, calc2, calc3, calc4, calc5],
                            [x, y, p, q, alpha],
                            # [((a+n)/2), ((b+m)/2), ((c+g)/2), ((d+h)/2-0.1), ((Point1.getAlpha(f)+Point2.getAlpha(f))/2-0.1)],
                            [pointFind.getX(), pointFind.getY(), pointFind.getP(), pointFind.getQ(), pointFind.getAlpha(f)],
                            # verify = False
                            set=True, particular=True, force=True
                            #        check=False
                              )
        # answer = numpy.ndarray(answer, dtype=float, order='F')
        return Point(round(parsFloat(answer[0]), 5), round(parsFloat(answer[1]), 5), round(parsFloat(answer[2]), 5), round(parsFloat(answer[3]), 5))
...
if __name__ == "__main__":
    m = Mathcad()
    length = 7
    pointsFind = [Point(0, 0, 1.732, 1, 3.464, 0),
                  Point(-0.475, -1.568, 2.115, 0.831, 6.015, 1.785),
    ...
    pw = pointsFind[1]
    answer2 = m.calcSymbWithAngle2(points2[1], points2[0], pw)
    print('\n', answer2,'\n', pw)
    ...
    # points2 = [Point(-2.241, -6.363, 2.757, 0.851), Point(-1.246, -5.841, 2.754, 0.766)]
    # pw = pointsFind[4]
    # answer2 = m.calcSymbWithAngle2(points2[1], points2[0], pw)
    # print('\n', answer2,'\n', pw)
    ...
    while(len(mass[i])!=0):
        ...
        if(len(mass)>=3 & i % 2 == 1):
            ...
            m2 = mass[i-1][0]
            # if(i>=3):
            #     m2 = pointsFind[i-leftNumber]
            find = pointsFind[i-leftNumber+1]
